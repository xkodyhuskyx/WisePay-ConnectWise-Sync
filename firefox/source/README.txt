...
Self-hosted updates
-------------------
Firefox checks the shared updates.json manifest published in the Firefox
extensions download directory. The file may list multiple add-ons. Each
published update for this add-on must reference a signed XPI whose internal
extension ID matches connectwise-to-wisepay@thefloofyfox.com.

Install the published Firefox extension from:
https://www.thefloofyfox.com/downloads/browser-extensions/firefox/connectwise-to-wisepay/connectwise-to-wisepay-1.3.2.xpi
...
Version 1.0.3
-------------
- Unlocks once per Firefox browser session.
- Decrypted credentials are held only in browser.storage.session memory.
- The session unlock is cleared automatically when Firefox closes.
...
- The cached company is cleared automatically when Firefox closes.


Version 1.1.0
-------------
- Added a second-stage billing address workflow.
- Remembers the last contact filled during the Firefox session.
...
- Added responsive styling for smaller Firefox windows.
