...
async function saveSettings(event) {
    ...
    try {
        ...
        // Request access while the form submission still carries a user gesture.
        // Background API calls cannot prompt for optional host permission later.
        await ensureConnectWiseHostPermission(
...
async function testConnection() {
    try {
        ...
        if (error instanceof TypeError) {
            ...
            showStatus(
                "The browser could not reach ConnectWise. Check the Base URL, API Path, network connection, and Chrome extension host permission.",
                true
            );
            return;
        }
...
async function ensureConnectWiseHostPermission(url) {
    ...
    if (!granted) {
        setFieldError(
            "base-url",
            "Chrome permission is required to connect to this host."
        );
        throw new Error(
...
async function resetAddon() {
    const confirmed = window.confirm(
        "Reset this extension? This removes saved credentials, the unlocked session, and all cached ConnectWise data from this Chrome profile."
    );
