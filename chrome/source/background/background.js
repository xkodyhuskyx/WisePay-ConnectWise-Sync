...
browser.runtime.onMessage.addListener((message, sender, sendResponse) => {
    ...
    if (message?.type === "getInlineContacts") {
        responsePromise = getInlineContacts(
            message.companyName,
            sender.url || sender.tab?.url
        );
    } else if (message?.type === "getInlineAddresses") {
        responsePromise = getInlineAddresses();
    } else if (message?.type === "markInlineContactSelected") {
        responsePromise = markInlineContactSelected(
            sender.url || sender.tab?.url,
            message.companyName,
            message.contact
        );
    ...
    Promise.resolve(responsePromise)
        ...
        .catch(error => {
            sendResponse({
                error: error?.message ||
                    "The Chrome service worker could not complete the request."
            });
        });

    // Keep the Chrome message channel open for the asynchronous response.
    return true;
});
...
async function getInlineContacts(companyName, transactionUrl) {
    try {
        ...
        if (!stored.addonUnlocked || !stored.unlockedConnectWiseCredentials) {
            return {
                error: "Unlock the extension from the Chrome toolbar first."
            };
        }
...
async function getInlineAddresses() {
    try {
        ...
        if (!stored.addonUnlocked || !stored.unlockedConnectWiseCredentials) {
            return { error: "Unlock the extension from the Chrome toolbar first." };
        }
...
async function connectWiseGet(endpoint, query, credentials) {
    ...
    if (!hasHostPermission) {
        throw new Error(
            "Chrome does not have permission to access the ConnectWise API host. Open the extension settings and save the credentials again to grant access."
        );
    }
