...
const setupRequiredSection = document.getElementById("setup-required-section");
...
const configureAddonButton = document.getElementById("configure-addon");
const developerNameElement = document.getElementById("developer-name");
const addonVersionElement = document.getElementById("addon-version");
...
let isCurrentUrlSupported = false;
...
configureAddonButton.addEventListener("click", () => browser.runtime.openOptionsPage());
themeToggleButton.addEventListener("click", () => window.addonTheme.cycle());
window.addEventListener("addonthemechange", updateThemeButton);
contactSearchInput.addEventListener("input", filterDisplayedItems);

async function initializePopup() {
    setExtensionMetadata();

    const localState = await browser.storage.local.get([
        "encryptedPasswordVerifier",
        "encryptedConnectWiseCredentials"
    ]);
    hasStoredCredentials = Boolean(
        localState.encryptedConnectWiseCredentials
    );

    if (
        !localState.encryptedPasswordVerifier &&
        !localState.encryptedConnectWiseCredentials
    ) {
        setSetupRequiredState();
        return;
    }
...
function setExtensionMetadata() {
    const manifest = browser.runtime.getManifest();
    const developerName =
        manifest.developer?.name || manifest.author || "";

    developerNameElement.textContent = developerName;
    addonVersionElement.textContent = `v${manifest.version}`;
    updateThemeButton();
}
...
function updateThemeButton() {
    ...
    themeToggleButton.querySelector("span").textContent = theme.icon;
    themeToggleButton.setAttribute("aria-label", `Theme: ${theme.label}`);
    themeToggleButton.title = `Theme: ${theme.label}`;
}

function setSetupRequiredState() {
    popupTitle.textContent = "Wise-Sync Guest Checkout Helper";
    companyNameElement.textContent = "Configure the add-on to get started.";
    document.getElementById("setup-required-title").textContent =
        "Setup Required";
    document.getElementById("setup-required-message").textContent =
        "You must set an add-on password and configure your ConnectWise API credentials before using this add-on.";
    setupRequiredSection.hidden = false;
...
function setCredentialsRequiredState() {
    popupTitle.textContent = "Wise-Sync Guest Checkout Helper";
    companyNameElement.hidden = true;
    document.getElementById("setup-required-title").textContent =
        "Credentials Required";
    document.getElementById("setup-required-message").textContent =
        "The add-on is unlocked. Configure your ConnectWise API credentials to continue.";
    setupRequiredSection.hidden = false;
    unlockSection.hidden = true;
...
async function restoreSessionState() {
    ...
    isSessionUnlocked = stored.addonUnlocked === true;
...
async function detectCurrentPageMode() {
    ...
    if (!tab?.id || !tab.url || !isSupportedWiseSyncUrl(tab.url)) {
        isCurrentUrlSupported = false;
        currentPageMode = "unsupported";
        return;
    }

    isCurrentUrlSupported = true;
    ...
    const results = await browser.scripting.executeScript({
        ...
        func: detectWiseSyncPageMode
    });

    const result = results.find(item => item.result?.mode);
    const immediateMode = result?.result?.mode ?? null;

    if (immediateMode) {
        currentPageMode = immediateMode;
        return;
    }

    const cached = await browser.storage.session.get(
        "lastWiseSyncInputState"
    );
    const inputState = cached.lastWiseSyncInputState;

    currentPageMode = inputState?.url === tab.url
        ? inputState.mode
        : "unsupported";
}

function detectWiseSyncPageMode() {
    function detectMode() {
        const companyField =
...
function applyPageMode() {
    ...
    contactsSection.hidden = true;
    setupRequiredSection.hidden = true;
    unsupportedSection.hidden = true;
    statusElement.textContent = "";
...
function setUnsupportedState() {
    ...
    if (isCurrentUrlSupported) {
        const paymentButtonLabel = document.createElement("span");
        paymentButtonLabel.className = "payment-method-button";
        paymentButtonLabel.textContent = "Guest Card Payment";
        unsupportedMessage.replaceChildren(
            "Select the ",
            paymentButtonLabel,
            " button to continue."
        );
    } else {
        unsupportedMessage.textContent = "This page is not supported.";
    }
    contactsSection.hidden = true;
    unlockSection.hidden = true;
    unlockedSection.hidden = false;
...
async function unlockAndAutoLoad() {
    ...
    try {
        ...
        const sessionState = { addonUnlocked: true };

        if (credentials) {
            sessionState.unlockedConnectWiseCredentials = credentials;
        }
...
async function unlockStoredCredentials(password) {
    ...
    if (!stored.encryptedConnectWiseCredentials) {
        if (!stored.encryptedPasswordVerifier) {
            throw new Error("An add-on password has not been configured.");
        }
...
function createContactCard(contact) {
    ...
    names.append(
        ...
        createContactField("Phone Number", contact.phoneNumber || "—")
    );
